...
class App extends Component {
  ...
  updateTimer = () => {
    const { time, gameEnded } = this.state
    if (time === 0) {
  ...
  handleThumbnailClick = (imageUrl) => {
    const { imageToMatch, score, time } = this.state
    if (imageUrl === imageToMatch) {
      const randomIndex = Math.floor(Math.random() * imagesList.length)
      this.setState({
        imageToMatch: imagesList[randomIndex].imageUrl,
        score: score + 1,
      })
    } else {
      this.endGame()
  ...
  render() {
    const { score, time, activeTab, imageToMatch, gameEnded } = this.state
    const filteredImages = imagesList.filter((item) => item.category === activeTab)
    ...
    return (
      <div className="App">
        ...
        <div className="tabs-container">
          {tabsList.map((tab) => (
            <button
              key={tab.tabId}
              type="button"
              className={activeTab === tab.tabId ? 'active-tab' : ''}
        ...
        <img src={imageToMatch} alt="match" />
        <div className="thumbnails">
          {filteredImages.map((item) => (
            <button
              key={item.id}
              type="button"
              onClick={() => this.handleThumbnailClick(item.imageUrl)}
            >
        ...
        {time === 0 ? (
          <div className="game-over">
            ...
            <p>YOUR SCORE: 0{score}</p>
            <img
              src="https://assets.ccbp.in/frontend/react-js/match-game-reset-icon.png"
              alt="reset"
            />
          </div>
        ) : null}
